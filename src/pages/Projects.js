...
function ProjectsPage() {
  ...
  useEffect(() => {
    setIsLoading(true);
    fetch("https://jjsolutions.rs/api/projectsapi.php?user_id=" + userId)
      .then((response) => {
  ...
  return (
    <div className={classes.wrap}>
      ...
      {loadedProjects.length > 0 ? (
        ...
        <NoProjectsCard newProject={createNewProject} />
      )}
      {newProjectModal && <Backdrop onCancel={closeCreateNewProject} />}
      {newProjectModal && (
        <NewProjectForm onCancel={closeCreateNewProject} user_id={userId} />
      )}
