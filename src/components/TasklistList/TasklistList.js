...
function TasklistList(props) {
  ...
  function removeFromList(removeId) {
    setNeededLists(
      neededLists.filter(
        (individualList) => individualList.list_id !== removeId
      )
    );
  }
  ...
  return (
    <div className={classes.wrap}>
      {neededLists &&
        neededLists.map((singleList) => {
          return (
            <TasklistCard
              key={singleList.list_id}
              onDelete={removeFromList}
              tasklist_name={singleList.list_name}
              tasklist_tasks={singleList.tasks}
              tasklist_id={singleList.list_id}
            />
          );
        })}
    </div>
