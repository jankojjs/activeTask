...
function TimeTracking(props) {
  ...
  const [formatedInitialTime, setFormatedInitialTime] = useState(
    new Date(initialTime * 1000).toISOString().substr(11, 5)
  );
  ...
  function isTrackingCheck() {
    setIsTracking(
      localStorage.getItem("active_task") === props.taskDetails.task_id
    );
  }
  ...
  function updateTimeRecord() {
    const url =
      "https://jjsolutions.rs/api/addtimeapi.php?time=" +
      counter +
      "&task_id=" +
      props.taskDetails.task_id +
      "&user_id=" +
    ...
    fetch(url)
      ...
      .then((body) => {
        if (body !== undefined) {
          setFormatedInitialTime(
            new Date((initialTime + parseInt(counter)) * 1000)
              .toISOString()
              .substr(11, 5)
          );
        } else {
    ...
    <div className={classes.wrap}>
      ...
      <div className={classes.bot}>
        <div className={classes.startTimer}>
          {isTracking ? (
            <span onClick={pauseClick}>
              <AiFillPauseCircle color={"red"} size={28} />
            </span>
          ) : (
            <span onClick={plusClick}>
              <AiFillPlayCircle color={"green"} size={28} />
            </span>
          )}
        </div>
        <div className={classes.botMid}>
          <span style={{ color: isTracking ? "red" : "" }}>{formatedTime}</span>
          <span onClick={resetTrackingHandler} className={classes.reset}>
            <AiOutlineClose size={22} onClick={resetTrackingHandler} />
          </span>
        </div>
