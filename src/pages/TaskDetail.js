...
function TaskDetailPage() {
  ...
  function fetchTaskHandler() {
    fetch(
      "https://jjsolutions.rs/api/singletaskapi.php?user_id=" +
  ...
  function fetchTaskPeople() {
    fetch("https://jjsolutions.rs/api/taskpeopleapi.php?task_id=" + taskId)
      .then((response) => response.json())
      .then((data) => {
        if (data.length === 0) {
  ...
  return (
    <div>
      {notLoading ? (
        <PageDetailCard taskObj={taskObj} people={peopleArr} />
      ) : (
        <div>Loading...</div>
      )}
    </div>
