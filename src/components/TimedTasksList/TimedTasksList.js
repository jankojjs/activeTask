          ...
          body.map((singleTasklton) => {
            if (newData.indexOf(singleTasklton.taskId) !== -1) {
              ...
              newData.push({
                ...
                days: [{ [singleTasklton.date]: singleTasklton.timeuploaded }],
              });
            }
          });
  ...
  return (
    <div>
      {fetchedTasks.map((singleFetchedTask) => {
        return (
          <TimedTaskRow
            key={singleFetchedTask.timetrackId}
            singleTaskInfo={singleFetchedTask}
            days={days}
            upstreamFunc={sendTimeToWeek}
            toggle={passToggle}
          />
        );
      })}
