  ...
  return (
    <span>
      <select
        ...
        ref={taskAsigneeInput}
      >
        <option value="">None</option>
        {initialAsigneeEmptyHandler && (
          <option value={props.asignee}>{props.asignee}</option>
        )}
        {props.peoples.map((singlePerson) => {
          if (
            singlePerson.username !== props.asignee &&
            singlePerson.username !== null
          ) {
            return (
              <option
                key={singlePerson.project_id}
                value={singlePerson.username}
              >
